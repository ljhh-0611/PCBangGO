...
public class PCroomDBHelper extends SQLiteOpenHelper {
    ...
    public PCroomDBHelper(Context context){super(context,"pcbang.db",null,1);}
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        ...
        String[] PCrooms = {"3POP","Arachne", "ANDSOON"};
        int[] rows = {6, 5, 4};
        int[] cols = {6, 2, 2};
        ...
        for (int i=0;i<3;i++) {
            /*String insert = String.format(
                    "INSERT INTO %s VALUES (null, %s, %s, %d, %d, %s, %s, %s);" , TABLE_NAME, PCrooms[i], rows[i], cols[i],
